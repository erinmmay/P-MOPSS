...
from m0_FullFrame import Bin_FullFrame
from m0_MasterFrame import MasterFrame
...
#             plt.show(block=False)
    ...
    for i in range(0,masks.shape[0]):
        ...
        data['obj'+str(int(i))]=np.empty([int(n_exp),int(topy-lowy),int(topx)-int(lowx)])*np.nan
        if Lflat==True:
            print('  ****obj ', i)
    ...
    data_2c=np.empty([int(2*ypixels/binny+ygap),int(xpixels/binnx)])*np.nan
    exp_cnt=0

    image_full=np.empty([int(2*ypixels/binny+ygap),int(4*xpixels/binnx+3*xgap)])*np.nan
        ...
        if file.endswith('.fits.gz'):
            ...
            if chip in top_chip:
                ...
                data_t=np.fliplr((fits.open(path+root+'c'+str(int(chip))+'.fits.gz')[0].data)
                                 [0:int(ypixels/binny),0:int(xpixels/binnx)])/top_gain[c]
                data_b=np.flipud((fits.open(path+root+'c'+str(int(bot_chip[c]))+'.fits.gz')[0].data)
                                 [0:int(ypixels/binny),0:int(xpixels/binnx)])/bot_gain[c]
                #print root, c, np.nanmedian(data_t), np.nanmedian(data_b)
                data_2c[0:int(ypixels/binny),:]=data_t
                data_2c[int(ypixels/binny+ygap):,:]=data_b
                del data_t
                del data_b
                if c==0:
                    image_full[:,0:int(xpixels/binnx)]=data_2c
                if c==1:
                    image_full[:,int(xpixels/binnx+xgap):int(2*xpixels/binnx+xgap)]=data_2c
                if c==2:
                    image_full[:,int(2*xpixels/binnx+2*xgap):int(3*xpixels/binnx+2*xgap)]=data_2c
                if c==3:
                    image_full[:,int(3*xpixels/binnx+3*xgap):]=data_2c
            exp_cnt+=(1./8.)
            if exp_cnt%1==0:
