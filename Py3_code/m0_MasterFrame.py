...
def MasterFrame(path,kind,SAVEPATH,binnx,binny):
    ...
    print(' ')
    full=np.empty([int(n_frames),4,int(2*ypixels/binny+ygap),int(xpixels/binnx)])*0.0
    exp=0
        ...
        if file.endswith('.fits.gz'):
            ...
            if chip in top_chip:
                ...
                data_b=np.flipud((fits.open(path+root+'c'+str(int(bot_chip[c]))+'.fits.gz')[0].data)
                                 [0:int(ypixels/binny),0:int(xpixels/binnx)])
                full[int(exp),c,0:int(ypixels/binny),:]=data_t
                full[int(exp),c,int(ypixels/binny+ygap):,:]=data_b
            exp+=1./8.
