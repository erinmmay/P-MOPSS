...
def BinWhite(SAVEPATH,midtime,start,end):
    ...
    for s in range(0,n_obj):
        ...
        for t in range(0,n_exp):
            ...
            while wav_arr[s,t,p]>wave_cntr+width_bin:
                ...
                wave_cntr+=width_bin
            while wave_cntr<bin_arr[-1] and p < pixels:
                counts=0
                err1=0
                err2=0
                ptne=0
                ...
                counter=0
                if p>0:
                    while (wav_arr[s,t,p-1]+wav_arr[s,t,p])/2. < wave_cntr+width_bin:
...
def BinLam(SAVEPATH,midtime,start,end,width):
    cnt_arr=np.load(SAVEPATH+'ShiftedSpec_All.npz')['data']
    wav_arr=np.load(SAVEPATH+'ShiftedSpec_All.npz')['wave']
