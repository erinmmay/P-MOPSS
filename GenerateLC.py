...
#W52- 2,3,5,7,8
    ...
    errs_w_t/=np.nanmean(LC[0:10])
    errs_w_p/=np.nanmean(LC[0:10])

    errs_cs_t/=np.nanmean(CS[0:10])
    errs_cs_p/=np.nanmean(CS[0:10])

    LC=LC/np.nanmean(LC[0:10])
    CS=CS/np.nanmean(CS[0:10])
    LCd=LC/CS
...
#                 LCd[t]=np.nanmedian(np.append(LCd[t-1],LCd[t+1]))
    ...
    errs_lcd_w_t/=np.nanmean(LCd[0:10])
    errs_lcd_w_p/=np.nanmean(LCd[0:10])
...
#                 LC_d[t,b]=np.nan
        ...
        LC_l[:,b]=LC_l[:,b]/np.nanmean(LC_l[0:10,b])
        CS_l[:,b]=CS_l[:,b]/np.nanmean(CS_l[0:10,b])
        LC_d[:,b]=LC_l[:,b]/CS_l[:,b]
    
        errs_lcd_l_t[:,b]/=np.nanmean(LC_d[0:10,b])
        errs_lcd_l_p[:,b]/=np.nanmean(LC_d[0:10,b])
